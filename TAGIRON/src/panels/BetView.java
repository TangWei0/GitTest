		...
		for (int i = 0; i < DIGITAL_PARAMETERS; i++) {
			for (int j = 0; j < HALF_DIGITAL_SIZE; j++) {
				...
				if (i == 0) {
					...
					if (j == 5) {
						BetLabel[i][j].setBorder(new LineBorder(tgrMain.tgrColorDisplay(tgrMain.ColorEnum.valueOfByName("Green").getId()), 5, true));
					} else {
						BetLabel[i][j].setBorder(new LineBorder(tgrMain.tgrColorDisplay(tgrMain.ColorEnum.valueOfByName("Red").getId()), 5, true));
					}
				} else {
					...
					if (j == 5) {
						...
						BetLabel[i][j].setBorder(new LineBorder(tgrMain.tgrColorDisplay(tgrMain.ColorEnum.valueOfByName("Blue").getId()), 5, true));
					}
				}
